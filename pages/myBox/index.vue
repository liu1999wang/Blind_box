	...
	<view class="container" style="padding-bottom: 160rpx;">
		<view class="header-tabs">
			<view class="tab-item" v-for="(item, index) in typeTextList" :class="{active:current==index}"
				...
				<text class="text">
					{{item}}
					<text style="color:red;font-size:24rpx;"><template v-if="statusTotal[types[index]]" >{{statusTotal[types[index]]}}</template></text>
				</text>
			</view>
		</view>
		...
		<view class="footer-c" v-if="current == 0 && !orderConfig.is_hide_deliver_btn">
			<template v-if="!isSelectMode">
				...
				<button class="select-box-btn enter-btn bg-cancel"  hover-class="hover"
...
<!-- 				<button class="select-box-btn enter-btn bg-orange" hover-class="hover"
					@tap="enterSelectMode('resale')" v-if="marketConfig.is_enabled">
					批量挂售
				</button> -->
		...
		<ResalePopup @cancel="hideResalePopup" @refresh="refresh" :packageSku="currentItem"
			:uuid="currentUuid" v-if="isShowResalePopup"></ResalePopup>

		<view class="mask" @tap="isReturnSaleSuccess=false" @touchmove.stop v-if="isReturnSaleSuccess">
			<view class="mask-content" @tap.stop>
				<text class="new-iconfont icon-close close-btn" @tap.stop="isReturnSaleSuccess=false"></text>
				<view class="title"><text class="new-iconfont icon-check"></text>分解成功</view>

				<view class="btn-c">
					<button class="btn cancel bg-purple" @tap="toPage('/pages/myRedpack/index')">余额</button>
		...
		<navigator url="/pages/myBox/locked">
			<view class="locked-float-btn global-shadow">
				<text class="new-iconfont icon-lock"></text>
				<view>{{statusTotal['locked'] || 0}}</view>
			</view>
		</navigator>
		<tab-bar :isselected="2"></tab-bar>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			successDeliver(obj) {
				...
				if (obj.deliver_type === 'custom_gateway') {
					this.refresh()
...
<style lang="scss">
		...
		.tab-item {
			...
			&.active {
				...
				.text {
					...
					font-size: 32rpx;
					font-weight: 900;
					border-bottom: 7rpx solid rgba(51, 51, 51, 1);
	...
	.swiper-wrapper {
		...
		scroll-view {
			...
			.order-list {
				overflow: hidden;
				padding: 0rpx 30rpx 260rpx 30rpx;
