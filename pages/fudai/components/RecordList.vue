...
<!-- 				<view class="item" :class="{actived: tag === 'free_order'}" @tap="setTag('free_order')">免单</view> -->
				<view class="item" :class="{actived: tag === 5}" @tap="setTag(5)">终极款</view>
				<view class="item" :class="{actived: tag === 4}" @tap="setTag(4)">欧皇款</view>
			</view>

			<scroll-view scroll-y class="scroll-view" @scrolltolower="fetchList">
				<view class="list-scope">
					<template v-for="(item, index) in list">
						<view hover-class="hover" class="item-scope global-shadow-2" v-if="tag=='all'||tag==item.level">
							...
							<view class="sku-c">
								...
								<view class="shang-title" :class="{strong: item.shang_type == 1}">{{item.level_title}}</view>
	...
	export default {
		...
		methods: {
			...
			initData() {
				...
				// return this.$http(`/fudais/${this.info.uuid}/record-total`, 'GET', {
