<template>
	<view class="scope-container" :style="'min-height: 100vh; background:#000000'">
		...
		<view style="position: relative;" >
			...
			<!-- 中赏记录 -->
			<image  @tap="isShowRankList = true" class="winning_record" src="https://www.hebide.cc/newfudaiimages/winning_record.png" mode="widthFix"></image>
		...
		<OpenBoxPopupTheme2 :doubleBoxCard="is_doubleBoxCard" :config="config" :levelskus="info.skus" :sku_level="info.sku_level" boxImgMode="square" :tryMode="isTryMode" v-if="isShowResultPopup" :order="order" @close="goBack" :boxImg="boxImage"></OpenBoxPopupTheme2>
		...
		<RecordList v-if="isShowRankList" @close="isShowRankList = false" :skuLevel="skuLevel" :info="info" :room="info.room"></RecordList>
...
<script>
	...
	export default {
		...
		methods: {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							uni.navigateTo({
								url:`/pages/try/detail?uuid=${this.info.uuid}`
							})
			...
			paySuccess (order, isTryMode = false,is_doubleBoxCard) {
				this.isTryMode = isTryMode
				this.order = order
				this.is_doubleBoxCard=is_doubleBoxCard
...
<style lang="scss" scoped>
	...
	.try_box{
		...
		width: 200rpx;
		height: 100rpx;
		background: url("https://www.hebide.cc/newimages/MF.png") no-repeat;
		z-index: 99;
		background-size: 100%;
...
.footer2-box{
	...
	.new-treasure_box{
		...
		width:375rpx;
		height: 60rpx;
		background: url("https://www.hebide.cc/newfudaiimages/new/sbk.png") no-repeat;
		background-size: 100%;
		navigator{
			width: 100%;
			height: 100%;
