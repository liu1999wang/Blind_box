<template>
	<view class="mask" @tap="cancel" @touchmove.stop>
		<view class="mask-content  animated bounceInUp" @tap.stop>
			...
			<view class="total"> <text>购买数量</text> <text>{{payTotal}}</text> <text>发，共计</text><text  style="font-size: 46rpx; font-weight: 200;">￥{{payTotal*info.money_price/100}}</text></view>
			...
			<view class="block">
							...
							<view class="arrow-right bold" @tap="isCouponPopup = true">
								...
								<template>
									...
									<text class="meta gray-text" >暂无可用优惠券</text>
								</template>
								<text class="new-iconfont icon-arrow-right"></text>
							</view>
						</view>
<!-- 						<view class="block">
							<view class="item-title">试玩红包:</view>
...
<!-- 						<view class="block" v-if="doubleBoxCard.is_show && doubleBoxCard.my_balance>0">
							<view class="item-title">{{doubleBoxCard.double_card_info.title}}</view>
							<view class="arrow-right bold">
